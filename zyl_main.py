...
def handle_query():
    try:
        ...
        task_queue.put((session_id, user_input))
        
        return jsonify({
            "session_id": session_id,
            "status": "processing",
            "result_url": f"/results/{session_id}"
        })
    except Exception as e:
...
if __name__ == '__main__':
    ...
    app.run(host=config.HOST, port=config.PORT, debug=config.ENV == Env.DEV, use_reloader=False)