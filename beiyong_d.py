...
class AIAgentApp(QWidget):
    ...
    def select_agent(self, agent_id):
        ...
        if agent_id == 1:
            ...
            # 构建app.py的完整路径和dan74目录路径
            dan74_dir = os.path.join(current_dir, 'dan74')
            app_path = os.path.join(dan74_dir, 'app.py')
            
            # 打印当前目录和目标目录（用于调试）
            # print(f"Current directory: {current_dir}")
            # print(f"Target directory: {dan74_dir}")
            # print(f"App path: {app_path}")
            
            # 确保dan74目录存在
            if not os.path.exists(dan74_dir):
                print(f"Error: Directory {dan74_dir} does not exist!")
                return
            
            # 检查所需的JSON文件是否存在
            required_files = [
                '北京大学食堂菜谱数据集-66eb503d54.json',
                '饮食健康关联知识图谱基础数据库-473840c6d1.json',
                '饮食健康问答数据集-4f5619f42b.json',
                '饮食健康问答数据集-7688752c2b.json'
            ]
            
            for file in required_files:
                file_path = os.path.join(dan74_dir, file)
                if not os.path.exists(file_path):
                    print(f"Error: Required file {file} not found at {file_path}")
            
            # 创建新的环境变量，继承当前环境
            env = os.environ.copy()
            # 设置PYTHONPATH以确保可以找到模块
            env['PYTHONPATH'] = current_dir
            
            # 使用Python解释器运行app.py，并设置工作目录为dan74
            process = subprocess.Popen(
                [sys.executable, app_path],
                cwd=dan74_dir,  # 设置工作目录
                env=env  # 设置环境变量
            )
